...
if __name__ == '__main__':
    ...
    ###########
    table_data = []
    table_data.append([Color('{autogreen}ID{/autogreen}'),Color('{autogreen}User{/autogreen}'),Color('{autogreen}Local{/autogreen}'),Color('{autogreen}Remote{/autogreen}'),Color('{autogreen}Status{/autogreen}'),Color('{autogreen}Port{/autogreen}'),Color('{autogreen}App{/autogreen}'),Color('{autogreen}Country{/autogreen}')])
    for conn in netstat():

        r = str(conn[3])[0:conn[3].index(":")]
        rIp = str(gi.country_code_by_addr(r))
        conn.append(rIp)
        ...
        if "root" not in conn:
            ...
            for i in range(0,8):
                conn[i] = Color('{autored}' + str(conn[i]) + '{/autored}')
            table_data.append(conn)
